...
namespace Blogs.DAL
{
	...
	public class SysManage //这里必须实现接口，否则工厂类创建接口报错
	{
		...
		public int GetMaxId()
		{          
			return DbHelperSQLite.GetMaxID("NodeID", "S_Tree");           
		}
		...
		public void UpdateNode(SysNode model)
		{
			...
			SQLiteParameter[] parameters = {
											new SQLiteParameter("@NodeID", SqlDbType.Int,4),
											new SQLiteParameter("@Text", SqlDbType.VarChar,100),
											new SQLiteParameter("@ParentID", SqlDbType.Int,4),										
											new SQLiteParameter("@Location", SqlDbType.VarChar,50),
											new SQLiteParameter("@OrderID", SqlDbType.Int,4),
											new SQLiteParameter("@comment", SqlDbType.VarChar,50),
											new SQLiteParameter("@Url", SqlDbType.VarChar,100),
											new SQLiteParameter("@PermissionID", SqlDbType.Int,4),
											new SQLiteParameter("@ImageUrl", SqlDbType.VarChar,100)};
			parameters[0].Value = model.NodeID;
			parameters[1].Value = model.Text;
			parameters[2].Value = model.ParentID;		
			parameters[3].Value = model.Location;
